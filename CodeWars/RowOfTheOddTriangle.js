// https://www.codewars.com/kata/5d5a7525207a674b71aa25b5/train/javascript

function oddRow(n) {
    let arr = []
    if (n == 1) return [1]
    if (n == 2) return [3, 5]
    if (n.length % 2 == 1) {
        arr.push((n * n) - 1)
        arr.push((n * n) + 1)
    } else {
        arr.push(n * n)
    }
    while (arr.length < n) {
        arr.unshift((arr[0]) - 2)
        arr.push((arr[arr.length - 1]) + 2)
    }
    return arr
}

console.log(oddRow(1), 'Answer: ', [1]);
console.log(oddRow(2), 'Answer: ', [3, 5]);

console.log(oddRow(13), 'Answer: ', [157, 159, 161, 163, 165, 167, 169, 171,
    173, 175, 177, 179, 181]);

console.log(oddRow(19), 'Answer: ', [343, 345, 347, 349, 351, 353, 355, 357,
    359, 361, 363, 365, 367, 369, 371, 373, 375, 377, 379]);

console.log(oddRow(41), 'Answer: ', [1641, 1643, 1645, 1647, 1649, 1651, 1653,
    1655, 1657, 1659, 1661, 1663, 1665, 1667, 1669, 1671, 1673, 1675, 1677,
    1679, 1681, 1683, 1685, 1687, 1689, 1691, 1693, 1695, 1697, 1699, 1701,
    1703, 1705, 1707, 1709, 1711, 1713, 1715, 1717, 1719, 1721]);